...
processFilter = function processFilter(filter, context) {
    ...
    transformHaving = function transformHaving(statements) {
        var propMap = {},
            props,
            maxProp,
            maxPropValues,
            maxPropLessStatements,
            whereInFuncStatement;

        _.eachStatement(
            statements,
            function (statement, index) {
                var prop = statement.prop;
        ...
        props
            .forEach(function (prop) {
                if (propMap[prop].length > maxPropValues.length) {
                    maxProp = prop;
                    maxPropValues = propMap[prop];
                }
            });

        maxPropLessStatements = _.rejectStatements(
            statements,
            function (statement) {
                return _.findStatement(
                    [
                        statement
                    ],
                    {
        ...
        if (maxPropLessStatements.length === 0) {
            return whereInFuncStatement;
        }

        return _
            .mergeStatements(
                {
                    statements: maxPropLessStatements
                },
                whereInFuncStatement
            ).statements;
    };
    ...
    filter.statements = transformHaving(filter.statements);
